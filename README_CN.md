...
**意图与范围分诊门（DNA 4）：** 未激活角色的领域请求在执行前即被拦截——模糊提示立即暂停，输出 2–3 个分支的决策卡（附推荐的 `@agent` / `/skill` 句柄）；琐碎任务直接执行，零分诊开销。

**展示与输出规范（DNA 6）：** 结构化与表格数据必须使用标准 Markdown 表格；流程与架构图必须使用 Mermaid；ASCII 仅限用于 UI 线框图/视觉原型、终端交互预览及无法用 Mermaid 表达的图形。
