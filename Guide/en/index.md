# Vespyr — Complete Guide

> [中文版本](../cn/index.md)

Welcome to the Vespyr comprehensive guide. Follow these chapters in order for a first-time setup, or jump to any section for reference.

## Chapters

| # | Chapter | What You'll Learn |
|---|---------|-------------------|
| 1 | [Installation](installation.md) | Prerequisites, `npx vespyr`, first initialization |
| 2 | [Getting Started](getting-started.md) | Your first project, invoking agents, basic pipeline |
| 3 | [Configuration](configuration.md) | `.agents/config.yaml`, custom agents |
| 4 | [Skills & Workflows](skills-and-workflows.md) | All slash commands, pipeline phases, skill anatomy |
| 5 | [Reference](reference.md) | Agent contracts, delegation, glossary, memory system |

## Quick Links

- **README** — [English](../../README.md) | [中文](../../README_CN.md)
- **Quick Reference** — [One-page overview](../../QUICK-REFERENCE.md)
- **Agent Hub** — [`.agents/agents/`](../../.agents/agents/)
- **Skills Catalog** — [`.agents/skills/`](../../.agents/skills/)
- **Workflow Spec** — [`.agents/workflow.md`](../../.agents/workflow.md)
- **Guardrails** — [`.agents/GUARDRAILS.md`](../../.agents/GUARDRAILS.md)
