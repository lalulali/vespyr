...
> [← 返回指南](index.md) | [上一章：配置](configuration.md) | [下一章：参考 →](reference.md)
...
| `/elicitation` | 98 种结构化提示引导，推动模型完善输出 |
| `/round-table` | 多智能体分阶段圆桌讨论（覆盖全 11 个阶段与 20 个智能体角色） |
...
| 命令 | 描述 |
|---------|-------------|
| `/shut-up` | 单次静默执行模式 — 直接执行任务，零主动批判与冗余废话 |
| `/review` | 独立代码审查与安全审计 |
| `/test` | 运行测试，分析失败并生成 QA 报告 |
