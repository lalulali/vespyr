...
| 智能体 | 拥有 | 不拥有 |
|-------|------|-------------|
| `@founder` | 战略决策、GO/RESHAPE/NO-GO | 实现细节 |
...
- **强制结论门禁 (`[NO-GO]` | `[RESHAPE]` | `[GO]`):**
  - **`[NO-GO]` (彻底否决):** 存在致命权衡或未验证的花哨需求。**否决零蓝图不变量 (Zero-Blueprint-on-NO-GO):** 严禁为被否决的前提生成实现计划、架构图或折中选项。
  - **`[RESHAPE]` (调整方向):** 底层需求合理，但机制臃肿或存在风险。剥离冗余并推荐零成本原生方案。
  - **`[GO]` (通过验收):** 经由实证数据或基准测试验证符合所有领域不变量。
...
5. **输出格式规范** — 结构化/表格数据必须使用标准 Markdown 表格（禁止 ASCII 表格）。图表与流程图必须使用 Mermaid。ASCII 仅保留用于 UI 线框图/视觉原型、终端模拟及无法用 Mermaid 表达的图形。
