...
`08-cross-cutting-utter-satisfaction-dna.md`.
...
```json
{
  ...
  "parameters": {
    "agent": {
      "type": "string",
      "description": "Agent name matching Vespyr persona (derived from .agents/agents/*.md at server start — never a hardcoded list)",
      "enum": ["founder", "product-manager", "product-designer", "architect", "tech-lead", "developer", "code-reviewer", "qa-engineer", "researcher", "user-researcher", "ux-researcher", "data-analyst", "security-engineer", "performance-engineer", "ml-ai-engineer", "ml-ai-ops", "shifu", "devops-engineer", "technical-writer", "reader", "writer", "executor", "memory-controller"]
    },
    "prompt": {
      "type": "string",
...
- [ ] **F3b.17** — Add MCP agent invocation to `develop/SKILL.md` Step 7 (QA gate): `@qa-engineer` is spawned via MCP tool, produces structured output with `verdict: GO|NO-GO|CONDITIONAL` — AND must write the `qa-signoff.md` file (signature + conditions + mode fields); the verdict string alone never advances the gate (F3b.31 + qa_check.js). Step 6 (code review): `@code-reviewer` spawned via MCP tool.
...
- **Credential hygiene (F3b.19, 09 R-add):** provider credentials are loaded from environment variables ONLY — never from harness config files that may contain keys. **Every output path redacts key/token/secret patterns** (no credential echo to stdout/stderr → agent context → memory). A no-credential-echo test is part of `test:agent-run`.
- **Timeout.** Default 30s for reasoning agents, 15s for I/O agents. Configurable via `--timeout`.
- **Provider auto-detection.** The CLI detects the active model provider from the harness configuration. Priority: `VESPYR_PROVIDER` env var → harness config (credentials still env-only) → `.agents/config/models.json` default.
...
```json
{
  ...
  "agent_tier_map": {
    ...
    "ml-ai-engineer": "balanced", "ml-ai-ops": "balanced", "shifu": "balanced",
    "technical-writer": "balanced"
  }
}
```

> **Config ownership:** `models.json` is the model-tier config (created by F3b.20); `.agents/config.yaml` is the behavior/flint config (Phase 0). Both live under `.agents/config/` — the earlier "model.json" typo is corrected.
...
- [ ] **F3b.19** — Create `.agents/scripts/agent_run.js` (~500-700 lines — the 350-line estimate was under-scoped; 3+ provider API clients + harness-config detection + credential redaction + batch mode): parses flags (`--agent`, `--agents`, `--prompt`, `--context`, `--model`, `--format`, `--timeout`), loads agent persona, builds prompt, calls LLM API, formats output. Supports single-agent and batch multi-agent (sequential) modes. Includes the credential-redaction requirement (env-only creds, no-echo test).
- [ ] **F3b.20** — Create `.agents/config/models.json` (~60 lines): model tier configuration with agent-to-tier mapping (names derived from `.agents/agents/*.md`).
- [ ] **F3b.21** — Implement provider auto-detection in `agent_run.js`: detect provider from harness config (opencode.json, Claude Code env, Cursor config) → fallback to `VESPYR_PROVIDER` env var → fallback to `models.json` default → hardcoded fallback. Credentials always come from env vars, never from config files.
...
- [ ] **F3b.24** — Create test: `npm run test:agent-run` — invokes `agent-run --agent founder --prompt "Is this a good idea: a CLI tool for running agents?" --format json`, verifies structured output, verifies model routing (executor uses fast model, founder uses premium), verifies timeout behavior, verifies batch mode (--agents founder,product-manager), verifies no credential echo in any output path.

**Estimate:** 14-18 hours (revised from 8-10 — provider auto-detection across harness configs is the rabbit hole)
...
- Solo-mode `qa-signoff.md` (from Phase 2 F2.23-F2.27) is marked `INVALID — solo mode` and cannot pass the QA hard gate (`Mode: solo` in the sign-off template fails `qa_check.js`).
- **Gate binding (round-table fix):** the phase-advance block keys on the mode RECORDED in `mode-log.json` (F3b.35) at selection time — not on a self-asserted watermark in output text, which a solo LLM can simply omit. The watermark remains as a user-facing label; the gate reads recorded state. `VESPYR_ALLOW_SOLO_DEV=1` is an experimentation override only. It never bypasses the T8 satisfaction validator or permits a release.
...
- [ ] **F3b.27** — Implement solo mode in `orchestrator_state.js`: block phase advance out of `development` when the mode RECORDED in `mode-log.json` (F3b.35) is solo — the gate reads recorded state, never a self-asserted watermark. Allow override via `VESPYR_ALLOW_SOLO_DEV=1` env var with explicit warning.
...
```json
{
  ...
  "satisfaction": {
    "state": "SATISFIED",
    "evidence_refs": ["artifacts/output/06-quality/qa-report.md"],
...
- [ ] **F3b.31** — Wire structured output into `orchestrator_state.js`: `check-verdict` subcommand reads structured output and extracts the consensus verdict. **Advisory only** — an MCP/structured verdict alone never advances a gate: `check-verdict` must require the `qa-signoff.md` file and delegate to `qa_check.js` (F2.26) for the actual gate decision (prevents the MCP QA path from bypassing the signature+file gate). `qa-signoff.md` generation (Phase 2 F2.23-F2.27) includes a hash of the structured output as metadata (pinned serialization) — metadata, never a gate input.
...
- [ ] **F3b.35** — Log mode selection to telemetry: every multi-agent operation writes to `artifacts/telemetry/mode-log.json` with `{timestamp, skill, phase, mode, agent_count, mechanism, harness, groupthink_risk}`. This log is the authoritative mode record — the orchestrator's solo-mode phase block reads it (F3b.27). Feeds into self-learning metrics (Phase 2 F2.15.a-c).
...
| `08-cross-cutting-utter-satisfaction-dna.md` | Define the shared state vocabulary, evidence contract, and release gate consumed by every mode | Low |
...
| M3: CLI Orchestrator | F3b.19-F3b.24 | 14-18 |
...
| **Total** | **41 items** | **49-63 hours** (revised from 43-55 — M3 was under-scoped) |

**Calendar:** ~3 weeks (can run in parallel with Plan 03a MCP integration EXCEPT M2 — F3b.13 extends 03a's server, so M2 must follow 03a Step 1; the shared `@vespyr/mcp` server means the two plans should sequence their server edits rather than edit concurrently).
...
| **Solo mode produces convincing but wrong outputs in dev/QA** | High | High | Solo mode is blocked from development/QA phases by default, gated on the mode recorded in `mode-log.json` (not a forgeable watermark). Mode watermark is non-suppressible as a user-facing label. |
...
| **Adapter or solo mode fabricates satisfaction** | Medium | High | Validate the structured satisfaction object, require evidence + witness fingerprints, gate on recorded mode (mode-log.json), keep release advancement blocked on incomplete or stale rows, and demote `check-verdict` to advisory (qa_check.js owns the gate). |
| **CLI provider detection leaks credentials** | Low | Medium | Credentials load from env only; every output path redacts key/token patterns; no-credential-echo test in `test:agent-run`. |
...
- [ ] MCP agent invocation tool (`vespyr_spawn_agent`)
- [ ] Headless CLI orchestration runner (`bin/agent-run.js`)
- [ ] Guarded solo mode fallback with mandatory devil's advocate

---

## Sign-Off

**@architect (Vera):** PENDING — Multi-agent orchestration protocol and harness adaptation review.  
**@tech-lead (Grant):** PENDING — Execution scheduled for v2.1.  
**@qa-engineer (Nina):** PENDING — Multi-agent harness matrix validation suite.
