...
UTTERLY SATISFIED validation defined in `08-cross-cutting-utter-satisfaction-dna.md`.
...
- [x] `.agents/config.yaml` with `step_tracking: off` default
- [x] `.agents/scripts/step_tracker.js` implementation (~260 lines)
- [x] `.agents/scripts/add-step-tracker-calls.js` helper implementation (95 lines)
- [x] `.agents/GUARDRAILS.md` step tracking documentation
- [x] Reference rollout across core skills (`shape-up`, `develop`, `design`, `validate-idea`, `launch`, `retro`)

---

## Sign-Off

**@tech-lead (Grant):** APPROVED — SATISFIED (2026-07-20). Scope: step tracking breadcrumbs implemented without heavy prompt token bloat.  
**@developer (Rex):** APPROVED — SATISFIED (2026-07-20). Scope: lightweight file-based step tracker validated across core skills.
