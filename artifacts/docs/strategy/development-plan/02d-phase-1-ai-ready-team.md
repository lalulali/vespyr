...
[`08-cross-cutting-utter-satisfaction-dna.md`](08-cross-cutting-utter-satisfaction-dna.md), not from an
...
- [x] F1.33.1 — Update `.agents/agents/product-manager.md`: add AI-PM capabilities to frontmatter (see §A.6), add `AC-AI-*` generation pattern, and AI eval metrics to charter.
- [x] F1.33.2 — Add Sarah's 4-Question Decision Gate to persona `## Decision Framework` section.
- [x] F1.33.3 — Add 5 advanced AI scoping dimensions to persona `## Charter` section (labeled "Future-Proof — v2.x+").
- [x] F1.33.4 — Update Sarah's `channeled_mentor` field and channeled mentors description with AI PM mentors (Marily Nika, Shreyas Doshi, Claire Vo).
- [x] F1.33.5 — Add `## Session Continuity (Mandatory)` section per 02b pattern.

### @ml-ai-engineer (Kai) — Rename + Upgrade — ~2.5 days
- [x] F1.34.1 — Rename `.agents/agents/ml-engineer.md` → `.agents/agents/ml-ai-engineer.md`.
- [x] F1.34.2 — Update content of `ml-ai-engineer.md` with upgraded v2 frontmatter (see §B.4), charter, sub-disciplines, hard rules.
- [x] F1.34.3 — Execute migration: update all 30+ cross-references listed in §B.5 (`@ml-engineer` → `@ml-ai-engineer`).
- [x] F1.34.4 — Rename `.agents/references/socratic/ml-engineer.md` → `.agents/references/socratic/ml-ai-engineer.md`.
- [x] F1.34.5 — Add to `build` (core) and `data-platform` (opt-in) squads.
- [x] F1.34.6 — Add to `agent-contracts.md`: owns model/prompt/RAG/eval; does NOT own production serving.
- [x] F1.34.7 — Define `model-approved-for-production.md` artifact template in `.agents/templates/architecture/`.
- [x] F1.34.8 — Add `## Session Continuity (Mandatory)` section per 02b pattern.

### @ml-ai-ops (Atlas) — New Persona — ~3 days
- [x] F1.35.1 — Create `.agents/agents/ml-ai-ops.md` (v2 frontmatter per §C.5, IDENTITY block, channeled mentor, icon, charter, hard rules, delegation contract with `@writer` for file output).
- [x] F1.35.2 — Document handoff rule from Kai → Atlas in both persona files (artifact-based, no direct call).
- [x] F1.35.3 — Add to `agent-contracts.md`: owns production serving, monitoring, rollback; does NOT own model/prompt development.
- [x] F1.35.4 — Add `## Session Continuity (Mandatory)` section per 02b pattern.

### @architect (Vera) — AI-Ready Upgrade — ~1.5 days
- [x] F1.36.1 — Update frontmatter: add AI-readiness capabilities to `capabilities` list (see §D.4).
- [x] F1.36.2 — Extend `.agents/agents/architect.md` with AI-ADR types and AI-specific system design checklist.
- [x] F1.36.3 — Add Vera's AI-ready checklist (§D.3) to the `## Checklist` section.
- [x] F1.36.4 — Update `channeled_mentor` field: add Chip Huyen + Martin Kleppmann (AI/data systems context).
- [x] F1.36.5 — Add AI security boundary design (prompt injection, model access control, PII-by-design) to charter.
- [x] F1.36.6 — Add `## Session Continuity (Mandatory)` section per 02b pattern (if not already added by 02b execution).

### @qa-engineer (Nina) — AI-Ready Upgrade — ~1.5 days
- [x] F1.37.1 — Update frontmatter: add AI-readiness capabilities to `capabilities` list (see §E.4).
- [x] F1.37.2 — Extend `.agents/agents/qa-engineer.md` with eval harness integration, non-deterministic AC verification, and AI release certification checklist.
- [x] F1.37.3 — Add golden eval dataset and red-team dataset ownership to Nina's charter.
- [x] F1.37.4 — Add post-launch AI regression monitoring to Nina's charter.
- [x] F1.37.5 — Update `channeled_mentor` field: add Shreya Shankar + Eugene Yan.
- [x] F1.37.6 — Add `## Session Continuity (Mandatory)` section per 02b pattern (if not already added by 02b execution).

### AGENTS.md & Cross-System Updates
- [x] F1.38.1 — Update `.agents/AGENTS.md`: update `@ml-engineer` row to `@ml-ai-engineer`, add `@ml-ai-ops` row, update agent count.
- [x] F1.38.2 — Update `06-phase-5-deeper-bench.md`: mark T1.10 and T1.10b as `[MOVED to 02d]`.
- [x] F1.38.3 — Add T8 evidence, revalidation, and release-gate requirements to the AI handoff artifacts and validators.
...
- [x] `@product-manager` (Sarah) upgraded to dual-capacity AI-PM with AC-AI-* criteria & 4-Question Gate
- [x] `@ml-ai-engineer` (Kai) renamed and upgraded with comprehensive AI & ML lifecycle charter
- [x] `@ml-ai-ops` (Atlas) new persona authored and integrated for production ML infrastructure
- [x] `@architect` (Vera) upgraded with AI-ADR templates and system design checklists
- [x] `@qa-engineer` (Nina) upgraded with AI eval harness & release certification checklists
- [x] Cross-system updates: `AGENTS.md`, `agent-contracts.md`, and memory continuity verified

---

## Sign-Off

**@product-manager (Sarah):** APPROVED — SATISFIED (2026-08-01). Scope: dual-capacity AI product management and non-deterministic AC generation.  
**@ml-ai-engineer (Kai):** APPROVED — SATISFIED (2026-08-01). Scope: AI & ML engineering charter, RAG, and eval scorecards.  
**@ml-ai-ops (Atlas):** APPROVED — SATISFIED (2026-08-01). Scope: production model deployment, monitoring, and rollback operations.  
**@architect (Vera):** APPROVED — SATISFIED (2026-08-01). Scope: AI architecture patterns and fallback path design.  
**@qa-engineer (Nina):** APPROVED — SATISFIED (2026-08-01). Scope: AI release certification and eval dataset management.
