...
**Phase 6 Loop Engineering status: PLANNED (v2.1 Scope — Not Started).**

- [ ] Autonomous loop execution engine (`/goal` primitive with iteration & token caps)
- [ ] Read-only stop-condition verifier (`@goal-verifier`)
- [ ] Event-driven automation system (`automation.js`, CI-failure triage)
- [ ] Loop state persistence and pause/resume engine (`loop-state.json`)

---

## Sign-Off

**@tech-lead (Grant):** PENDING — Loop execution and maker/checker separation review.  
**@architect (Vera):** PENDING — Worktree isolation and loop state architecture review.  
**@qa-engineer (Nina):** PENDING — Stop-condition verification and goal-verifier testing.
