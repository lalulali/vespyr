...
1. **Core DNA 4 ("The Intent & Scope Triage Gate / Anti-Assumed Execution"):** Add an unconditional, token-dense ($\le 120$ tokens) invariant to `.agents/references/vespyr-dna.md` and `AGENTS.md`. If a request is ambiguous or multi-track ($0.50 \le C < 0.85$), halt and force the **2–3 Track Fork** card (<100 tokens). If unambiguously specific ($C \ge 0.85$), directly adopt the specialist persona banner and execute.
...
**Position:** Phase 1 (vespyr 2.0.0) sub-plan — 12th in the `02*` series, positioned alongside `02j-phase-1-evals-and-agnostic-harness.md` and immediately prior to `03-phase-2-enablement.md`.
...
│   5. Result: Zero actionable insight, high token waste, zero hypothesis testing.       │
│                                                                                        │
...
│   ├── DNA 4: Intent & Scope Triage Gate (Anti-Assumed Execution, ≤120 tokens)          │
...
Add to `.agents/references/vespyr-dna.md` and embed in `AGENTS.md` (Strict Attention Budget: $\le 120$ tokens):
...
2. **Multi-Track / Ambiguous ($0.50 \le C < 0.85$):**
   ...
   - **The 2–3 Track Fork:** Output a concise (<100 tokens) card presenting 2–3 divergent decision tracks with recommended `@agent` and `/skill` handles.
3. **Dispatch:**
...
- [ ] **Task 02k.1 — Vespyr Core DNA 4 Codification**
  ...
  - **Action:** Add `## DNA 4: Intent & Scope Triage Gate (Anti-Assumed Execution)` with the high-confidence direct adoption banner ($C \ge 0.85$), the 2–3 Track Fork (<100 tokens, $0.50 \le C < 0.85$), and strict attention budget ($\le 120$ tokens).
  - **Verify:** Verify `AGENTS.md` and `vespyr-dna.md` contain identical, token-dense DNA 4 definitions.
...
- [ ] **Task 02k.6 — Author `evals/suites/intent-routing.json`**
  ...
  - **Action:** Scaffold test fixtures (including `intent-04` trivial task decoy trap) and assert Tier 0 token ceiling / regex pattern gates for un-persona'd requests.
...
1. **DNA 4 Active:** `vespyr-dna.md` and `AGENTS.md` contain token-dense `DNA 4: Intent & Scope Triage Gate` ($\le 120$ tokens).
...
- [ ] **Task 02k.1:** Codify `DNA 4: Intent & Scope Triage Gate` ($\le 120$ tokens, dual-route $C \ge 0.85$ vs $0.50 \le C < 0.85$) in `.agents/references/vespyr-dna.md` and `AGENTS.md`.
- [ ] **Task 02k.2:** Expand §22 in `.agents/GUARDRAILS.md` to ban broad horizontal survey fallbacks and mandate the 2–3 Track Fork UX protocol.

### Phase 2: Agent Persona Socratic Hardening
- [ ] **Task 02k.3:** Update 12+ reasoning personas (`@researcher`, `@user-researcher`, `@ux-researcher`, `@architect`, `@tech-lead`, `@developer`, `@product-manager`, `@product-designer`, `@security-engineer`, `@performance-engineer`, `@ml-ai-engineer`, `@devops-engineer`) with explicit Socratic rejection of underspecified briefs.

### Phase 3: Skill Step 0 Gating & Deterministic Script Verification
- [ ] **Task 02k.4a:** Author `step-00-scope-and-decision-anchoring.md` across core skills (`/explore-idea`, `/unpack-problem`, `/develop`, `/design`, `/validate-idea`).
- [ ] **Task 02k.4b:** Update `.agents/scripts/step_tracker.js` to assert `scope_locked: true` before advancing past Step 0 (exit code 1 if bypassed).

### Phase 4: Multi-Harness Sync & Documentation
- [ ] **Task 02k.5:** Update `CLAUDE.md`, `.cursorrules`, `README.md`, `README_CN.md`, `artifacts/memory/project-context.md`, and `artifacts/memory/active-decisions.md`.

### Phase 5: Automated Evals & Decoy Trap Benchmarking
- [ ] **Task 02k.6:** Author `evals/suites/intent-routing.json` in `vespyr-eval` asserting Tier 0 regex patterns and Tier 1 G-Eval calibration anchors across positive triage, single-track direct adoption, explicit bypass, and trivial decoy tasks.
