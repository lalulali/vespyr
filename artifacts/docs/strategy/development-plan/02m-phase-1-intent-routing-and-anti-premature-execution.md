...
- [x] **Task 02k.1:** Codify `DNA 4: Intent & Scope Triage Gate` ($\le 120$ tokens, dual-route $C \ge 0.85$ vs $0.50 \le C < 0.85$) in `.agents/references/vespyr-dna.md` and `AGENTS.md`. **[EXECUTED 2026-08-28]** Evidence: `rg -c "DNA 4: Intent & Scope Triage Gate" AGENTS.md CLAUDE.md .agents/references/vespyr-dna.md` → 1 each, identical block; trivial-tier (C < 0.50) included to match eval decoy intent-04.
  **[RECORD CORRECTION → REMEDIATED 2026-08-28, review roundtable]:** the ≤120-token budget was **[FALSIFIED]** — block measured 243 tokens via `tools/eval/lib/tokenizer.js`; the 2026-08-23 CHANGES-REQUESTED (`active-decisions.md` [DECISION] Round Table Review: 6 Dev Plans) had not been applied. Same-day remediation: block compressed to **119 tokens** (measured), Ladder Level 3 precedence clause added, harness-dispatch line moved to AGENTS.md §Invocation, eval `maxTokens` gates lowered 300→120. "Dual-route" → tri-route (disclosed).
