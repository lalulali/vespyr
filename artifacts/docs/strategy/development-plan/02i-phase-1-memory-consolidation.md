...
>
> **[HARDENING ADDENDUM 2026-08-25]** Audit residuals closed: lock heartbeat freshness evicts live-pid zombie holders after 15s without corruption (R2); tamper-aware mtime-guarded release can never delete a successor's lock (R1); §11.1 `<1,000` injected-token ceiling now enforced in `memory_filter.js` with truncation reporting; `compaction_guard.js` thresholds aligned to §2.2 spec — no contradicting numbers remain on disk. Evidence: `tests/test_memory_fixes.test.js` F8–F10; full suite **158/158**. Remaining open: **R-2 re-stamps** (all seven sign-off rows; gates Phase 2 entry) and **R-3 owner enforcement-mode decision** (Option A harness write-hooks vs Option B read-time scrubbing — undecided by owner; `AGENTS.md` direct-edit instruction stands pending that ruling).
