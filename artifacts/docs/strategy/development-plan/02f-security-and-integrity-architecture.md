...
**Position:** Phase 1 (vespyr 2.0.0) sub-plan — 7th in the `02*` series, between `02e-agentskills-standardization.md` and `03-phase-2-enablement.md`. Phase 0 (01-*) = v1.7 dev (shipped); Phase 1 is the current position. This is the last Phase 1 gate before Phase 2 (v2.1) begins, because Phases 2+ assume a trustworthy content pipeline.

**Gate reviews:** Round table 2026-08-08 (@security-engineer, @devops-engineer, @architect, @ml-ai-engineer, @tech-lead). Victor issued CHANGES REQUESTED ×2 during scoping, all closed by Grant's amendments; **Victor sign-off: APPROVED — SATISFIED** (scope: authoring plan F1.39–F1.57 + DoD gate only; future implementation requires fresh audit). Document-instantiation review (round 2): CHANGES REQUESTED ×3 (Victor/Grant/Axel) + amendments (Vera); consolidated by Grant and applied. Round 3 verdict: pending re-review.
...
Every surface below must carry: attack narrative + severity + mapped control with owning F-number. Orphans go to `09-risk-register.md` with an owner — never silently dropped.
...
### 3.2 Trust tiers (T0–T3, defined in ADR-01)
...
- Bootstrap decision pinned in F1.48: TOFU with out-of-band check OR pinned bootstrap anchor — resolved from ADR-02, recorded with owner + rationale. **Honesty constraint:** fail-closed cannot hold on first install under pure TOFU — require a pinned anchor or documented out-of-band verification on first install; the pin store enforces everything after.
...
- **Legacy backfill decision required:** existing memory predates tagging — either backfill or declared out-of-scope with sunset date (unenforceable T2 trust day one otherwise)
...
Every scanner rule maps to a label; every label has a rule or an explicit bypass entry.
...
1. **`security/supply-chain-audit-spec.md`** (human-readable)
2. **`audit-spec.json`** (machine-readable, importable by scan.js) containing:
   - **Adapter-registry pattern table:** harness dir → config files to inspect, derived from `bin/install.js`'s per-harness adapter registry (opencode.json, `.claude/settings.json`, `.cursor/hooks/hooks.json`, `.kiro/*`, etc.) — **no hardcoded harness names anywhere**
...
| Attribute | Requirement |
|---|---|
| Rules | One per taxonomy label (§7); add GH-1 git-hooks rule (parse `.git/config` + `.husky/` executable scan), lstat symlink targets, path-traversal filenames, shell-metachar in interpolated IDs, write-time secret scan |
...
- **FP budget:** enforced as a **NEW-FINDINGS-ONLY** gate against a frozen baseline corpus; triage owner: @security-engineer + @tech-lead. **Denominator defined: FP rate := informational findings ÷ total baseline-corpus executions per run (per harness shape), measured on the frozen corpus only; new findings labeled and never auto-baselined** (Nina). Absolute ≤1% is informational, not a gate
...
## 11. Build Items (F1.39–F1.57, 62h)
...
| F1.52 | Risk-register updates + cross-refs to 09-risk-register.md + 10-implementation-specs.md; release-pipeline handoff owner (**@devops-engineer + @security-engineer**; signer: Sigstore keyless cosign, fallback GPG key in CI secrets; CI fails when a release tag lacks a freshly generated manifest) | 3h | F1.48, F1.51 |
...
**Dependency-critical path** (only true dependency edges): `39 → 40 → 51 → 43 → 46 → 47 → 54 → 53` = **26h** (2+2+2+4+4+5+5+2), with F1.48 (4h, finishes at 14h) parallel off-path and 52→53 trailing (F1.52 finishes at 17h < 26h). F1.56 (~14h) and F1.57 (~19h) carry slack and do not move the path. Do NOT conflate the authoring order with the dependency path.
...
- `02e-agentskills-standardization.md` (format parity)
- One existing ADR + the ADR index (none in repo yet — create adr-001… as needed)
- `09-risk-register.md` + `10-implementation-specs.md`
...
|---|---|
| `09-risk-register.md` | Threat-model orphans + new risks (S-series surfaces) land here with owners; R16 (hooks), R18 (witness), R37 (migration) are adjacent |
| `10-implementation-specs.md` | `vespyr verify`/scan.js/validate_frontmatter specs ultimately implement here; new scripts referenced from F1.47/F1.48 |
| `02e-agentskills-standardization.md` | Skills spec compliance (frontmatter/schema) is the T1 validation layer this plan's T1 handling consumes |
| `07-harness-integration.md` | Supply-chain pinning decision (curl from main → pinned tags) codified in §5.1 |
| `14-utter-satisfaction-dna.md` | T8 gate: @security-engineer sign-off recorded before 02f closes (DoD #14) |
| ADR index | 4 new ADRs (adr-NNN) registered |
...
New entries to add to `09-risk-register.md` (owning F-numbers from §3):
...
**@security-engineer (Victor):** APPROVED — SATISFIED (2026-08-08, round-1 sign-off of F1.39–F1.55). Scope: the authoring plan + DoD gate for 02f — **not** any future implementation of the scanner/verify tooling, which requires a fresh audit. Round-2 review (document instantiation) CHANGES REQUESTED ×5; amendments applied. Round-3 review: APPROVED — SATISFIED (non-blocking residuals folded in: R50–R52 owners, S16 scan.js network scope). Round-4 verdict: pending re-review after Grant/Kai/Nina fixes (path 25h, sum 60h, per-path enforcement matrix, recall measurement, CI mechanics, counting protocol).
