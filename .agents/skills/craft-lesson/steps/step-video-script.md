...
delegation:
  ...
  writes: "@writer (artifacts/output/teaching/video-script.md)"
  runs: none
output_contract:
...
Delegate reading `artifacts/output/teaching/knowledge-map.md` to **`@reader` (Page)**. Convert module concepts into engaging narrative video segments.
...
Delegate writing the video script to **`@writer` (Quill)**:
...
- **Writes:** `@writer` (Quill) for writing `artifacts/output/teaching/video-script.md`.
- **Runs:** none.
