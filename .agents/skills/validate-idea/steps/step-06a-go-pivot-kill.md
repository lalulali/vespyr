...
The verdict is a parseable line: `GO / RESHAPE / NO-GO — <one-line reason>`. GO requires every one of:
...
**GO** — All four verdict-contract criteria met (behavioral demand evidence, quantified status-quo cost, days-shippable wedge, no unresolved contradiction). No partial credit: interest-not-behavior or unquantified cost makes `[GO]` unavailable — issue RESHAPE.

**RESHAPE** — The core problem is real, but this approach is wrong. Propose a reframed direction. Re-run this skill with the revised framing.

**NO-GO** — Doesn't survive scrutiny. No demand, no urgency, premises don't hold. Document why. Don't soften it.

Every verdict includes one concrete **next action** — a specific real-world step, not "go research this."

## Output
Verdict (GO/RESHAPE/NO-GO) with rationale, alternatives assessment, and next action.
