...
- **Concrete outcome** — the artifact or result this `/design` run produces
- **Boundary constraints** — platform invariants, design-system constraints, breakpoints in/out of scope
- **Target decision** — the decision this design work informs

## 2. Decision Fork
- **Unambiguous (C ≥ 0.85):** proceed to mode routing (create / edit / validate).
- **Ambiguous / multi-track (0.50 ≤ C < 0.85):** HALT. Emit the 2–3 Track Fork card (<100 tokens) — e.g., full create flow vs edit of existing spec vs heuristic-validate only. Each track names its recommended `@agent` + `/skill` handles. Wait for selection. Zero artifact scaffolding to `artifacts/output/` before selection (GUARDRAILS: Anti-Premature Execution).
...
```bash
node .agents/scripts/step_tracker.js scope-lock --skill design --track "<create|edit|validate>"
node .agents/scripts/step_tracker.js begin --skill design --step 0
```
...
```bash
node .agents/scripts/step_tracker.js complete --skill design --step 0
```
