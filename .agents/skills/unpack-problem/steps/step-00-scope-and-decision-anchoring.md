...
- **Concrete outcome** — the artifact or result this `/unpack-problem` run produces
- **Boundary constraints** — what is explicitly in and out of scope
- **Target decision** — the decision this problem exploration informs

## 2. Decision Fork
- **Unambiguous (C ≥ 0.85):** proceed to Step 1.
- **Ambiguous / multi-track (0.50 ≤ C < 0.85):** HALT. Emit the 2–3 Track Fork card (<100 tokens) — e.g., guided vs automated vs combination mode, or divergent problem framings. Each track names its recommended `@agent` + `/skill` handles. Wait for selection. Zero artifact scaffolding to `artifacts/output/` before selection (GUARDRAILS: Anti-Premature Execution).
...
```bash
node .agents/scripts/step_tracker.js scope-lock --skill unpack-problem --track "<track-name>"
node .agents/scripts/step_tracker.js begin --skill unpack-problem --step 0
```
...
```bash
node .agents/scripts/step_tracker.js complete --skill unpack-problem --step 0
```
