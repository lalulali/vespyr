...
### Automated mode
`@product-manager` and `@user-researcher` auto-draft all four artifacts by simulating user perspectives based on the problem statement. Run the sub-skill workflows inline (same analysis logic, agent-driven rather than user-driven). Produce all four outputs in parallel. Add a `[AUTO-DRAFT]` label to each file for transparency.

### Combination mode (default)
Run automated mode first to produce drafts. Then walk the user through each artifact for review and refinement. Ask for each: "Does this match what you're seeing? What would you change?" Update artifacts after each refinement.
