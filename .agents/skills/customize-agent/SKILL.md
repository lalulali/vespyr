...
- When you want to change the agent's identity or role itself (create a new agent with `/create-agent`, or edit the agent source directly)
- When the change is about a skill, not an agent (use `/customize-skill`)
...
Override files live in `.agents/custom/<name>.toml` and are never touched by upgrades. A defaults file (`.agents/agents/<name>/customize.toml`) is optional — `merge_customization.js` treats a missing defaults file as empty, so an override alone is valid.

The merge script combines defaults + override and prints the merged result as JSON for verification. The override file wins on conflicts.

> [!IMPORTANT]
> **Known gap:** the merged output is a validated preview only. No runtime component currently consumes `.agents/custom/*.toml` when an agent loads — until that wiring exists, overrides are declarations, not applied settings. Do not claim an override changed an agent's behavior.
...
Inspect `.agents/agents/<name>/customize.toml` (if available) or the agent's markdown definition to identify target fields:
...
Create or update `.agents/custom/<agent>.toml`.
