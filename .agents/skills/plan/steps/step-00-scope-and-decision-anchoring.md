...
- **Concrete outcome** — the execution plan artifact this `/plan` run produces
- **Boundary constraints** — what is explicitly in and out of scope
- **Target decision** — the decision this plan informs

## 2. Decision Fork
- **Unambiguous (C ≥ 0.85):** proceed to Step 1.
- **Ambiguous / multi-track (0.50 ≤ C < 0.85):** HALT. Emit the 2–3 Track Fork card (<100 tokens) — e.g., full build plan vs single-slice plan vs dependency-spike-first. Each track names its recommended `@agent` + `/skill` handles. Wait for selection. Zero artifact scaffolding to `artifacts/output/` before selection (GUARDRAILS: Anti-Premature Execution).
...
```bash
node .agents/scripts/step_tracker.js scope-lock --skill plan --track "<track-name>"
node .agents/scripts/step_tracker.js begin --skill plan --step 0
```
...
```bash
node .agents/scripts/step_tracker.js complete --skill plan --step 0
```
