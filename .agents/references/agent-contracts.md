...
| `@developer` | Code implementation, unit tests, bug fixes, refactoring, **motion framework & `MO-###` implementation** | Architecture, design fidelity, product priorities |
...
| `@researcher` | Market analysis, competitive landscape, technology trends, **motion market/library landscape** | User research, UX, product strategy |
| `@user-researcher` | User interviews, personas, JTBD analysis | Market research, UX evaluation, product metrics |
| `@ux-researcher` | Usability evaluation, journey mapping, **binding design-versus-accessibility decisions**, **motion usability/accessibility research** | Visual design, code, market research |
...
Motion is a **capability**, not a separate agent. Ownership (per the row entries above):

- **`@product-designer`** owns the motion *spec* — `motion-spec.md`, motion tokens in `design.md`, and the complete motion prompts (`MO-###`) for `@developer`. Loads `.agents/references/motion/motion-design-guidelines.md` on demand.
- **`@developer`** owns the motion *framework and code* — implements every `MO-###` prompt verbatim, loads `.agents/references/motion/motion-implementation-guidelines.md` on demand. Under-specified prompts are flagged back to `@product-designer`, never improvised.
