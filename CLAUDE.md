...
All agents leverage the localized text-based memory system located in `artifacts/memory/`:
*   `project-context.md` — Tracks codebase stack, constraints, and active preset
*   `active-decisions.md` — Captures critical design choices
*   `lessons-learned.md` — Stores engineering insights and bugs fixed

## 🛡️ Guardrails

All agents follow the safety and conflict resolution rules in `.agents/GUARDRAILS.md`. Do not delete or modify files inside `.agents/`, `.claude/`, or `artifacts/memory/` without understanding the system.
