...
*   **Shutdown Completion Logging**: After saving deliverables, execute (or request `@executor` to execute) `node .claude/scripts/orchestrator_state.js complete --agent <name> --artifact <relative-path>` to update the state file and advance the pipeline.
...
4. **When approved:**
   ...
   - Update the relevant `.claude/workflow.md` files — these contain the workflow documentation.
